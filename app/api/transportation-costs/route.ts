...
async function scrapeFlightPrices(
  origin: string,
  destination: string,
): Promise<TransportationCosts["flights"] | null> {
  try {
...
async function scrapeTrainPrices(
  origin: string,
  destination: string,
): Promise<TransportationCosts["trains"] | null> {
  try {
...
async function scrapeBusPrices(
  origin: string,
  destination: string,
): Promise<TransportationCosts["buses"] | null> {
  try {
    const distance = calculateDistance(origin, destination);
...
async function getCurrentFuelPrices(): Promise<
  TransportationCosts["selfDrive"] | null
> {
  try {
...
async function getCurrentTaxiRates(): Promise<
  TransportationCosts["taxis"] | null
> {
  try {
...
function _calculateTransportationCost(
  origin: string,
  destination: string,
  mode: string,
  ...
  switch (mode.toLowerCase()) {
    ...
    case "selfdrive":
      return {
        cost:
          pricing.selfDrive.fuelPerKm * distance +
          pricing.selfDrive.rentalPerDay,
        duration: `${Math.round(distance / 50)} hours`,
