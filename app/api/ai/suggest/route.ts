...
function sanitizeJsonCandidate(candidate: string) {
  ...
  txt = txt.replace(/"([^"]*)"([^"]*)"([^"]*)"/g, (_match, p1, p2, p3) => {
    return `"${p1}\\"${p2}\\"${p3}"`;
  });
...
function extractJson(text: string) {
        ...
        for (const pattern of patterns) {
          ...
          if (matches && matches.length > 0) {
            ...
            if (suggestions.length > 0) {
              console.log(
                `Successfully extracted ${suggestions.length} suggestions from truncated response`,
              );
              return { suggestions };
            }
          }
    ...
    () => {
      ...
      if (suggestionsMatch) {
        ...
        if (matches && matches.length > 0) {
          ...
          if (suggestions.length > 0) {
            console.log(
              `Successfully extracted ${suggestions.length} suggestions from incomplete array`,
            );
            return { suggestions };
          }
        }
  ...
  try {
    ...
    for (const pattern of patterns) {
      ...
      if (suggestionMatches && suggestionMatches.length > 0) {
        ...
        if (suggestions.length > 0) {
          console.log(
            `Successfully extracted ${suggestions.length} suggestions using fallback method`,
          );
          return { suggestions };
        }
      }
  ...
  console.error(
    "Failed to extract valid JSON from:",
...
function extractPartialSuggestions(text: string): any[] {
  ...
  try {
    ...
    if (suggestionsMatch) {
      ...
      const suggestionMatches = suggestionsText.match(
        /\{[^{}]*"destination"[^{}]*\}/g,
      );
    ...
    if (matches && matches.length > 0) {
      for (const match of matches) {
        try {
          ...
          if (
            parsed.destination &&
            !suggestions.find((s) => s.destination === parsed.destination)
          ) {
            suggestions.push(parsed);
          }
        } catch (_e) {}
...
async function retryWithMinimalSchema(requestData: any, maxPlaces: number) {
  ...
  if (!budgetINR)
    return NextResponse.json({ error: "Missing budgetINR" }, { status: 400 });
  ...
  const _interestsList = Array.isArray(interests)
    ? interests.join(", ")
    : interests.join(", ");
...
- CRITICAL COST REQUIREMENTS:
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a travel planner. Output ONLY valid JSON. No markdown, no explanations. Keep responses concise.",
          },
    ...
    return NextResponse.json(
      { error: "Retry failed", details: String(err?.message || err) },
      { status: 500 },
    );
  }
}
...
Rules: Valid JSON only. Cost <= ₹${budgetINR}. Realistic costs. Keep responses concise.`;
  ...
  try {
    ...
    if (!parsed?.suggestions) {
      ...
      if (isTruncated) {
        errorMessage +=
          " - Response appears to be truncated. Try reducing the number of places requested.";
        ...
        if (fallbackSuggestions.length > 0) {
          console.log(
            `Returning ${fallbackSuggestions.length} suggestions extracted from truncated response`,
          );
          return NextResponse.json({
            suggestions: fallbackSuggestions,
            warning:
              "Response was truncated, but some suggestions were successfully extracted",
        ...
        if (partialSuggestions.length > 0) {
          console.log(
            `Returning ${partialSuggestions.length} partial suggestions from truncated response`,
          );
          return NextResponse.json({
        ...
        return await retryWithMinimalSchema(
          {
            budgetINR,
            days,
            preferredLocation,
