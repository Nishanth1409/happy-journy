...
function isNonIndiaCountry(placeName: string): boolean {
  ...
  return NON_INDIA_COUNTRIES.some(
    (country) =>
      normalized === country ||
...
export async function isPlaceInIndia(placeName: string): Promise<boolean> {
  try {
    ...
    if (!searchResponse.ok) {
      console.error(
        `Wikipedia search failed for ${placeName}:`,
    ...
    const _pageTitleLower = pageTitle.toLowerCase();

    // Check if the page title itself is a non-India country
    if (isNonIndiaCountry(pageTitle)) {
      console.log(
        `Rejected ${cleanPlaceName}: Page title "${pageTitle}" is a non-India country`,
      );
      return false;
    }
    ...
    if (snippet.includes("india") || snippet.includes("indian")) {
      ...
      if (
        !snippet.includes("indian american") &&
    ...
    if (!pageResponse.ok) {
      console.error(
        `Wikipedia page fetch failed for ${pageTitle}:`,
    ...
    if (isNonIndiaCountry(title)) {
      console.log(
        `Rejected ${cleanPlaceName}: Page title "${title}" is a non-India country`,
      );
      return false;
    }
    ...
    for (const country of NON_INDIA_COUNTRIES) {
      if (title === country || title.includes(country)) {
        ...
        if (
          title.startsWith(country) ||
          title.endsWith(country) ||
          title === country
        ) {
          console.log(
            `Rejected ${cleanPlaceName}: Title "${title}" indicates non-India country "${country}"`,
          );
          return false;
        }
      }
    ...
    for (const country of NON_INDIA_COUNTRIES) {
      ...
      const countryPattern = new RegExp(
        `\\b(in|of|from|located in|situated in|city in|town in|state of|country of|capital of|largest city in|in the|of the)\\s+(the\\s+)?${country.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`,
        "i",
      );
      if (countryPattern.test(textToCheck)) {
        console.log(
          `Rejected ${cleanPlaceName}: Text indicates location in "${country}"`,
        );
        return false;
      }

      // Check for patterns like "New York, United States" or "London, UK"
      const titleCountryPattern = new RegExp(
        `(,|\\s+in\\s+|,\\s+)(the\\s+)?${country.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`,
        "i",
      );
      if (titleCountryPattern.test(title)) {
        console.log(
          `Rejected ${cleanPlaceName}: Title "${title}" contains non-India country "${country}"`,
        );
        return false;
      }
    ...
    if (page.coordinates && page.coordinates.length > 0) {
      ...
      if (
        ...
        console.log(
          `Rejected ${cleanPlaceName}: Coordinates (${coord.lat}, ${coord.lon}) are outside India`,
        );
        return false;
      }
    }
    ...
    console.log(
      `Rejected ${cleanPlaceName}: Could not confirm location is in India`,
    );
    return false;
  } catch (error) {
